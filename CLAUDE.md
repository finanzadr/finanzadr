...
- Listmonk (self-hosted, `listmonk.juliolab.dev`) for newsletter signups, reached through `api/newsletter.js` — a same-origin proxy, because the Listmonk server sends no CORS headers so a direct browser `fetch` fails at preflight. The list UUID lives only in that file.
